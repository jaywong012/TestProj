...
import { setCurrentPage, setEditDetail, setPages, setPosts, setSearchKey } from "@/features/redux/slicers/postSlice";
...
const PostList = ({ loading, handleDelete, getPosts }) => {
  ...
  const facebookUser = useSelector((state) => state.socialAccessInfo.userDetails.facebook);
      ...
      <>
        ...
        <td className="text-overflow-ellipse max-width-100 text-center">
          {(() => {
            ...
            return post.isShared ? "Yes" : <Button onClick={() => handleCheckRetweet(post)}>Verify</Button>;
          })()}
        </td>
        <td className="text-overflow-ellipse max-width-100 text-center">
          {post.lastSavedTime}
